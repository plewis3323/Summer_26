...
An LLM that can call your fitting code is a different instrument from one that can only talk about it — this week you wire a model directly to functions like the ones you wrote in Phase 1. HTTP, JSON, and API-key hygiene were taught in Week 23; this week recaps them in the API-call setting and adds the piece Week 23 could not: **the model is not a trusted user.** Prompt injection and tool-layer authorization are part of the week, not a footnote.

## Objectives

- Recap HTTP, JSON, and API keys (Week 23); keep the key out of your code and logs.
...
- **Refuse unauthorized tool calls** even when the model's text asks for them (prompt injection).
...
- Simon Willison on prompt injection (any recent roundup) — the attack is "the model is following instructions that are not yours."

## Exercises

See `exercises.md` (built into the notebook when the week starts). Eight exercises,
...
error recovery via `is_error`, parallel tool calls, schema-enforced extraction, a
cost ledger, and a prompt-injection case blocked at the tool layer.

## Deliverable

`week37/` package: `tools.py` (schemas + implementations, including path/allowlist
checks), `loop.py` (the agentic loop), saved transcripts of E3–E5 and E8, and the
printed cost table under budget.
