...
(intuition only; the full derivation opens Phase 3). Week 20 is the mini-project: an image classifier with a tabular-features baseline —
EMCal photon vs merged-π⁰ (default, fully explained) or a public image task with
the same protocol.
...
**Month-end deliverable:** image classifier with a CNN-vs-tabular
comparison and an error analysis (physics-aware if you took the EMCal track).
